...
select 
 e.item_number, e.order_qty, e.picked_qty, e.qty_needed, e.pm_qty, e.bs_qty, e.scrb_qty, e.stg_qty, e.total_qty, e.location_id, e.hu_id,
...
from 
 (
  select  *,
   (CASE
     WHEN runningTotal - actual_qty <= qty_needed  OR (rowNumber = 1 AND (runningTotal > (qty_needed))) THEN 'PULL'
...
where ceiling((order_qty/@hday)*@dos) - pm_qty > 0
	 ...
	 AND (i.location_id LIKE @bld1 or i.location_id like @bld2) -- Bulk locations (SFS only or include RB/SEV) 
	 ) c
	) d
where (CASE
     WHEN runningTotal - actual_qty <= qty_needed  OR (rowNumber = 1 AND (runningTotal > (qty_needed))) THEN 'PULL'
